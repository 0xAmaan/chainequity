...
function log(message: string, color: 'blue' | 'green' | 'yellow' | 'red' | 'gray' = 'gray') {
  const colorFn = {
    blue: chalk.blue,
    green: chalk.green,
    yellow: chalk.yellow,
...
async function main() {
  ...
  try {
    runCommand('curl -s -X POST -H "Content-Type: application/json" --data \'{"jsonrpc":"2.0","method":"eth_blockNumber","params":[],"id":1}\' http://127.0.0.1:8545', 'Check Anvil connection');
  ...
  const addressMatch = deployOutput.match(/Deployed GatedEquityToken at: (0x[a-fA-F0-9]{40})/);
  if (!addressMatch) {
    log('✗ Could not find contract address in deployment output', 'red');
    log('Checking broadcast logs...', 'gray');

    try {
      const broadcastFiles = execSync('ls -t broadcast/DeployGatedEquity.s.sol/31337/*.json', { encoding: 'utf-8' }).trim().split('\n');
      const latestBroadcast = broadcastFiles[0];
      const broadcastData = JSON.parse(readFileSync(latestBroadcast, 'utf-8'));

      const contractAddress = broadcastData.transactions
        .find((tx: any) => tx.contractName === 'GatedEquityToken')
        ?.contractAddress;
  ...
  runCommand(`bun run cli mint ${testAddresses[0]} 1000`, 'Mint 1000 tokens to address #1');
  await sleep(500);
  runCommand(`bun run cli mint ${testAddresses[1]} 500`, 'Mint 500 tokens to address #2');
  await sleep(500);
  runCommand(`bun run cli mint ${testAddresses[2]} 250`, 'Mint 250 tokens to address #3');
  await sleep(500);
  ...
  log('⚠ Note: Indexer will sync historical events and then listen for new ones', 'yellow');
  log('  Press Ctrl+C to stop the demo and indexer\n', 'yellow');

  runCommand('bun run indexer > backend/logs/indexer.log 2>&1 &', 'Start indexer daemon');
  ...
  const jsonOutput = runCommand('bun run cli captable --format json', 'Generate JSON cap table');
  ...
  const csvOutput = runCommand('bun run cli captable --format csv', 'Generate CSV cap table');
  ...
  console.log(chalk.white('  • Query historical cap table: bun run cli captable --block <number>'));
